	...
	window.Media = {
		...
		moveStates : function(to_form, field) {
			...
			if (to_form) {
                ...
                //Developer Enchancement, Allows the use of FlowPlayer MP3 audio
                if (src.match(/(.+)(.*)\.mp3/)) {
                	var flashVars = "config={'clip':{'url':'" + src + "','autoPlay':false},'plugins':{'controls':{'autoHide':false,'fullscreen':false}}}"
                    src = '<?php echo $root; ?>tiny_mce/plugins/media/player/player.swf';
                    
                    data.width = 640;
                    data.height = 30;
                    data.type = 'flash';
                    data.params.flashvars = flashVars;
                    ...
                    setVal('flash_flashvars', flashVars); 
                }
                
                //Developer Enchancement, Allows the use of FlowPlayer MP4 video
                if (src.match(/(.+)(.*)\.mp4/)) {
                	var flashVars = "config={'clip':{'url':'" + src + "','autoPlay':false},'plugins':{'controls':{'autoHide':false}}}"
                    src = '<?php echo $root; ?>tiny_mce/plugins/media/player/player.swf';
                ...
                //Developer Enchancement, Allows the use of FlowPlayer F4V video
                if (src.match(/(.+)(.*)\.mp3/)) {
                	var flashVars = "config={'clip':{'url':'" + src + "','autoPlay':false},'plugins':{'controls':{'autoHide':false}}}"
                    src = '<?php echo $root; ?>tiny_mce/plugins/media/player/player.swf';
